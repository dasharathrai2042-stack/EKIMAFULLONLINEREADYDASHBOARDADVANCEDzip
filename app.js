...
(function(){
  function escLast(v){return String(v??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}
  function qrRowsLast(type){return (data[type]||[]).map(r=>`<option value="${r.id}">${escLast(r.code||r.employeeId||r.billNo||r.quoteNo||r.name||r.person||r.supplier||r.id)}</option>`).join("")}
  function qrScreenLast(){return `<div class="wrap"><div class="head"><div><h1>QR Entry Center</h1><div class="page-note">Scan or type an item code, customer code, employee ID, bill number or saved record ID.</div></div></div><div class="panel qr-entry-panel"><div class="qr-scan-row"><input id="qr-lookup" autofocus placeholder="Scan QR value or type code…" onkeydown="if(event.key==='Enter')lookupQR()"><button class="btn" onclick="lookupQR()">Find Record</button><button class="btn outline" onclick="startQRInput()">Focus Scanner</button></div><div id="qr-result" class="qr-result"><span class="muted">Matching records will appear here.</span></div></div><div class="panel"><h3>Generate QR</h3><div class="qr-generate-grid"><select id="qr-type">${["items","customers","suppliers","attendance","sales","purchases","quotations"].map(t=>`<option value="${t}">${schemas[t]?.title||t}</option>`).join("")}</select><select id="qr-record">${qrRowsLast("items")}</select><button class="btn" onclick="generateSelectedQR()">Generate QR</button></div><div id="qr-image"></div></div></div>`}
  function formatScreenLast(){const f=data.settings.formats||{},n=data.settings.numbering||{};return `<div id="format-editor-final" class="panel"><h3>Editable Document Formats & Auto Numbering</h3><div class="settings-grid"><label>Invoice title<input id="fmt-invoice" value="${escLast(f.invoiceTitle||"SALES INVOICE")}"></label><label>Purchase title<input id="fmt-purchase" value="${escLast(f.purchaseTitle||"PURCHASE INVOICE")}"></label><label>Quotation title<input id="fmt-quotation" value="${escLast(f.quotationTitle||"QUOTATION")}"></label><label>Invoice prefix<input id="fmt-inv-prefix" value="${escLast(n.invoicePrefix||"INV")}"></label><label>Purchase prefix<input id="fmt-pur-prefix" value="${escLast(n.purchasePrefix||"PUR")}"></label><label>Payment prefix<input id="fmt-pay-prefix" value="${escLast(n.paymentPrefix||"PAY")}"></label><label>Footer text<input id="fmt-footer" value="${escLast(f.footer||"Thank you for your business.")}"></label><label>Left signature label<input id="fmt-left" value="${escLast(f.signatureLeft||"Prepared By")}"></label><label>Right signature label<input id="fmt-right" value="${escLast(f.signatureRight||"Approved By")}"></label></div><button class="btn" onclick="saveFormatSettings()">Save Format Settings</button></div>`}
  const previousLast=window.render;
  window.render=function(){
    if(page==="qrentry"){document.getElementById("title").textContent="QR Entry Center";document.getElementById("content").innerHTML=qrScreenLast();const type=document.getElementById("qr-type");if(type)type.onchange=()=>{document.getElementById("qr-record").innerHTML=qrRowsLast(type.value)};return}
    const result=previousLast();
    if(page==="settings"&&!document.getElementById("format-editor-final")){const wrap=document.querySelector("#content .wrap");if(wrap){const panel=document.createElement("div");panel.innerHTML=formatScreenLast();wrap.appendChild(panel)}}
    return result;
  };
})();
...
(function(){
  ...
  window.renderSpecialERP=function(target){
    if(target==="health")return {title:"Business Health",html:health3()};
    if(target==="cashflow"||target==="finance")return {title:target==="finance"?"Finance Management":"Cash Flow",html:cashflow3()};
    if(target==="attendance")return {title:"Attendance",html:attendance3()};
    if(target==="daybook")return {title:"Day Book",html:daybook3()};
    if(target==="auditTrail")return {title:"Audit Trail",html:`<div class="wrap"><div class="head"><h1>Audit Trail</h1>${action3("auditTrail")}</div><div class="panel">${tableRows3(data.auditTrail,[["date","Date"],["time","Time"],["user","User"],["action","Action"],["type","Module"],["oldValue","Old Value"],["newValue","New Value"]])}</div></div>`};
    return null;
  };
...
(function(){
  ...
  addFields("sales",[["billNo","Bill No","text"],["insurance","Insurance Amount","number"],["legguard","Leg Guard Amount","number"],["product","Product","text"]]);
   addFields("purchases",[["address","Address","text"],["mobile","Contact Number","tel"]]);
  ...
  function formV2(type,record){
    const s=schemas[type],id=record?.id||"",party=type==="sales"?"person":type==="purchases"?"supplier":null;
     if(type==="sales"||type==="purchases"){
       const identity=type==="sales"?[["person","Name","text"],["place","Address","text"],["mobil","Contact Number","tel"]]:[["supplier","Name","text"],["address","Address","text"],["mobile","Contact Number","tel"]];
       const visibleKeys=identity.map(x=>x[0]);
       const hidden=s.fields.filter(([k])=>!visibleKeys.includes(k)).map(([k,,t])=>`<input id="f_${k}" type="hidden" value="${esc2(record?.[k]??"")}">`).join("");
   ...
   window.editRecord=function(type,id){
     if(type!=="sales"&&type!=="purchases")return oldEditRecordV2(type,id);
     const record=(data[type]||[]).find(r=>r.id===id);if(!record)return;
     document.getElementById("content").innerHTML=`<div class="wrap"><div class="head"><div><h1>Edit ${schemas[type].title}</h1><div class="page-note">Edit the customer or supplier identity, then update the spreadsheet entry.</div></div></div><div class="panel">${formV2(type,record)}<div id="entry-grid"></div></div></div>`;
     window.renderEntryGrid?.();window.scrollTo({top:0,behavior:"smooth"});
   };
  ...
  window.render=function(){
    const c=document.getElementById("content");
    if(page==="qrentry"){document.getElementById("title").textContent="QR Entry Center";const qrTypes=["items","customers","suppliers","attendance","sales","purchases","quotations"],opts=t=>(data[t]||[]).map(r=>`<option value="${r.id}">${esc2(r.code||r.employeeId||r.billNo||r.quoteNo||r.name||r.person||r.supplier||r.id)}</option>`).join("");c.innerHTML=`<div class="wrap"><div class="head"><div><h1>QR Entry Center</h1><div class="page-note">Scan or type an item code, customer code, employee ID, bill number or saved record ID.</div></div></div><div class="panel qr-entry-panel"><div class="qr-scan-row"><input id="qr-lookup" autofocus placeholder="Scan QR value or type code…" onkeydown="if(event.key==='Enter')lookupQR()"><button class="btn" onclick="lookupQR()">Find Record</button><button class="btn outline" onclick="startQRInput()">Focus Scanner</button></div><div id="qr-result" class="qr-result"><span class="muted">Matching records will appear here.</span></div></div><div class="panel"><h3>Generate QR</h3><div class="qr-generate-grid"><select id="qr-type">${qrTypes.map(t=>`<option value="${t}">${schemas[t]?.title||t}</option>`).join("")}</select><select id="qr-record">${opts("items")}</select><button class="btn" onclick="generateSelectedQR()">Generate QR</button></div><div id="qr-image"></div></div></div>`;document.getElementById("qr-type").onchange=e=>document.getElementById("qr-record").innerHTML=opts(e.target.value);return}
    const special=window.renderSpecialERP?.(page);if(special){document.getElementById("title").textContent=special.title;c.innerHTML=special.html;return}
    if(page==="imports"){c.innerHTML=importPage();document.getElementById("title").textContent="Import Center";return}
    if(page==="dashboard"){document.getElementById("title").textContent="Management Dashboard";c.innerHTML=dashboardV2();draw();return}
    if(page==="users"){document.getElementById("title").textContent="Users / Staff";c.innerHTML=usersPageEnhanced();return}
    if(page==="settings"){document.getElementById("title").textContent="System Settings";c.innerHTML=brandingPage();const f=data.settings.formats||{},n=data.settings.numbering||{};const panel=document.createElement("div");panel.id="format-editor-final";panel.className="panel";panel.innerHTML=`<h3>Editable Document Formats & Auto Numbering</h3><div class="settings-grid"><label>Invoice title<input id="fmt-invoice" value="${esc2(f.invoiceTitle||"SALES INVOICE")}"></label><label>Purchase title<input id="fmt-purchase" value="${esc2(f.purchaseTitle||"PURCHASE INVOICE")}"></label><label>Quotation title<input id="fmt-quotation" value="${esc2(f.quotationTitle||"QUOTATION")}"></label><label>Invoice prefix<input id="fmt-inv-prefix" value="${esc2(n.invoicePrefix||"INV")}"></label><label>Purchase prefix<input id="fmt-pur-prefix" value="${esc2(n.purchasePrefix||"PUR")}"></label><label>Payment prefix<input id="fmt-pay-prefix" value="${esc2(n.paymentPrefix||"PAY")}"></label><label>Footer text<input id="fmt-footer" value="${esc2(f.footer||"Thank you for your business.")}"></label><label>Left signature label<input id="fmt-left" value="${esc2(f.signatureLeft||"Prepared By")}"></label><label>Right signature label<input id="fmt-right" value="${esc2(f.signatureRight||"Approved By")}"></label></div><button class="btn" onclick="saveFormatSettings()">Save Format Settings</button>`;c.querySelector(".wrap")?.appendChild(panel);return}
    ...
    const s=schemas[page];if(!s)return;document.getElementById("title").textContent=s.title;c.innerHTML=`<div class="wrap"><div class="head"><div><h1>${s.title}</h1><div class="page-note">Type to search customers, models and products. Use the table bill action for print-ready sales bills.</div></div>${actionV2(page)}</div><div class="panel">${canV2("edit")?formV2(page):"<p class='muted'>View and export access only.</p>"}</div><div class="panel"><div class="toolbar"><input class="search" placeholder="Search Name, Mobile, Code, PAN/VAT…" oninput="filterTable(this.value)"><span class="muted">${(data[page]||[]).length} record(s)</span></div><div id="tbl">${operationalTable(page)}</div></div></div>`;if(page==="sales")partySearchV2("f_person","list_customers","customers");if(page==="purchases")partySearchV2("f_supplier","list_suppliers","suppliers");if((page==="sales"||page==="purchases")&&document.getElementById("form")){const panel=document.getElementById("form").closest(".panel");if(panel&&!document.getElementById("entry-grid")){const div=document.createElement("div");div.id="entry-grid";panel.appendChild(div);window.renderEntryGrid?.()}}
  };
})();
